...
while i < num_samples:
    #TODO
    c = 0 if np.random.random() < p_C(0) else 1
    s = 0 if np.random.random() < p_S_given_C(0, c) else 1
    r = 0 if np.random.random() < p_R_given_C(0, c) else 1
    w = 0 if np.random.random() < p_W_given_S_R(0, s, r) else 1
    if w == 1:
        samples[i] = c
        i += 1
    else:
...
for i in range(num_samples):
    # sampling from p(R | C, S, W)
    state[2] = 0 if np.random.random() < p_R_given_C_S_W[state[0], state[1], 0, state[3]] else 1
    # sampling from p(S | C, R, W)
    state[1] = 0 if np.random.random() < p_S_given_C_R_W[state[0], 0, state[2], state[3]] else 1
    # sampling from p(C | S, R)
    state[0] = 0 if np.random.random() < p_C_given_S_R[0, state[1], state[2]] else 1
